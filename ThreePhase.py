...
def check1(self, set180, set90, set45):
    ...
    combined = [x + y + z for x, y, z in zip(set180, set90, set45)]
        ...
        for j in set90:
            for k in set45:
                ...
                if total == closest:
                    ...
                    return {'phase1': i.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row1': row1, 'att1': att1, 'phasediff1': phasediff1, 'source1': 's180', 'phase2': j.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row2': row2, 'att2': att2, 'phasediff2': phasediff2, 'source2': 's90', 'phase3': k.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row3': row3, 'att3': att3, 'phasediff3': phasediff3, 'source3': 's45', 'total': total.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP)}


def check2(self, set180, set90, set225):
    dec.getcontext().prec = 6
    combined = [x + y + z for x, y, z in zip(set180, set90, set225)]
    closest = min(combined, key=lambda x: abs(dec.Decimal(x) - dec.Decimal(self.targetphase)))
    for i in set180:
        for j in set90:
            for k in set225:
                ...
                if total == closest:
                    ...
                    print("The closest solution is: ")
                    print(row1, row2, row3)
                    return {'phase1': i.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row1': row1, 'att1': att1, 'phasediff1': phasediff1, 'source1': 's180', 'phase2': j.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row2': row2, 'att2': att2, 'phasediff2': phasediff2, 'source2': 's90', 'phase3': k.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row3': row3, 'att3': att3, 'phasediff3': phasediff3, 'source3': 's225', 'total': total.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP)}


def check3(self, set180, set45, set225):
    dec.getcontext().prec = 6
    combined = [x + y + z for x, y, z in zip(set180, set45, set225)]
    closest = min(combined, key=lambda x: abs(dec.Decimal(x) - dec.Decimal(self.targetphase)))
    for i in set180:
        for j in set45:
            for k in set225:
                ...
                if total == closest:
                    ...
                    print("The closest solution is: ")
                    print(row1, row2, row3)
                    return {'phase1': i.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row1': row1, 'att1': att1, 'phasediff1': phasediff1, 'source1': 's180', 'phase2': j.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row2': row2, 'att2': att2, 'phasediff2': phasediff2, 'source2': 's45', 'phase3': k.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP), 'row3': row3, 'att3': att3, 'phasediff3': phasediff3, 'source3': 's225', 'total': total.quantize(dec.Decimal('.001'), rounding=dec.ROUND_HALF_UP)}


def check4(self, set90, set45, set225):
    dec.getcontext().prec = 6
    combined = [x + y + z for x, y, z in zip(set90, set45, set225)]
    closest = min(combined, key=lambda x: abs(dec.Decimal(x) - dec.Decimal(self.targetphase)))
    for i in set90:
        for j in set45:
            for k in set225:
